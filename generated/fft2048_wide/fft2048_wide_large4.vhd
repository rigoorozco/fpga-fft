...
use work.fft2048_wide_ireorderer4;
use work.fft2048_wide_oreorderer4;
...
entity fft2048_wide_large4 is
	generic(dataBits: integer := 24; twBits: integer := 12; inverse: boolean := true);
	port(clk: in std_logic;
		din: in complex;
		phase: in unsigned(22-1 downto 0);
		twMultEnable, inTranspose, outTranspose: in std_logic;
		dout: out complex);
end entity;
architecture ar of fft2048_wide_large4 is
	signal din1: complex;
	signal ph: unsigned(22-1 downto 0);
	signal ibreorder_dout, twMult_dout, ireorder_dout, core_dout, oreorder_dout, obreorder_dout, twOut, twOutRaw: complex;
	signal ibreorder_phase, tw_phase, ireorder_phase, core_phase, oreorder_phase, obreorder_phase: unsigned(22-1 downto 0);
	signal twX, twY, twX1, twY1: unsigned(11-1 downto 0);
	signal twFineY, twFineY1: unsigned(2-1 downto 0);
	signal twIA, twIANext, twIB, twIBNext: unsigned(22-1 downto 0);
	signal twAddr, twAddr0: unsigned(22-1 downto 0);

	signal twAddrUpper: unsigned(12-1 downto 0);
	signal twAddrLower, twAddrLower0: unsigned(10-1 downto 0);
	signal twDataLower, twDataUpper: complex;

	signal romAddrUpper: unsigned(12-4 downto 0);
	...
	constant cumDelay_core: integer := cumDelay_ireorder + 8192;
	constant cumDelay_oreorder: integer := cumDelay_core + 2286;
	constant cumDelay_obreorder: integer := cumDelay_oreorder + 8192;
begin
	...
	ireorder: entity fft2048_wide_ireorderer4
		generic map(dataBits=>dataBits)
		port map(clk=>clk,
				phase=>ireorder_phase(13-1 downto 0),
				din=>twMult_dout,
				dout=>ireorder_dout);

	core: entity fft2048_wide
		generic map(dataBits=>dataBits, twBits=>twBits, inverse=>inverse)
		port map(clk=>clk,
				phase=>core_phase(11-1 downto 0),
				din=>ireorder_dout, dout=>core_dout);

	oreorder: entity fft2048_wide_oreorderer4
		generic map(dataBits=>dataBits)
		port map(clk=>clk,
				phase=>oreorder_phase(13-1 downto 0),
